...
if (-not $Maya) {
    ...
    & python -m pytest "$repo\tests\test_blend.py" "$repo\tests\test_settings.py" "$repo\tests\test_radial_geom.py" "$repo\tests\test_media.py" "$repo\tests\test_transcode.py" "$repo\tests\test_catalogue.py" "$repo\tests\test_usage.py" "$repo\tests\test_install.py" "$repo\tests\test_startup_ui.py" -q
    exit $LASTEXITCODE
}
